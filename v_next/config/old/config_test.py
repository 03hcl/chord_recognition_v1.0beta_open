...
class ConfigTest(Config):
    def __init__(self, root_dir: str, *args, **kwargs):
        super(ConfigTest, self).__init__(
            ...
            train=TrainConfig(batch_size=1000, number_of_epochs=60, progress_epoch=1, temporary_save_epoch=5,
                              optuna_number_of_trials=100, optuna_pre_trials=0,
                              optuna_pruner=PercentilePruner(200 / 3, n_startup_trials=5, n_warmup_steps=30)),
            # optimizer=ModuleConfig(Adam, lr=1e-3, eps=5e-5),
            # train=TrainConfig(batch_size=1000, number_of_epochs=100, progress_epoch=1, temporary_save_epoch=5),
